...
export const handler = async (request: Request): Promise<Response> => {
  try {
    ...
    if (DEBUG) {
      console.log(
        `specialities handler looking for name: ${name}, level: ${maxLevel}`
      );
    }
    ...
    if (name) {
      ...
      if (found) {
        if (DEBUG) {
          console.log(`Found speciality: ${found.name}`);
        ...
        if (DEBUG) {
          console.log(`No speciality found with name: ${name}`);
          console.log(
      ...
      return new Response(
        JSON.stringify({
          availableItems: allItems.map((item) => ({
            name: item.name,
            levels: item.levels.map((level) => level.level),
          })),
