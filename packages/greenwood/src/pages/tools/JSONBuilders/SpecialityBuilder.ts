...
export default class SpecialityBuilderPage extends HTMLElement {
  private formData = {
    name: "",
    levels: [] as unknown[],
  };
  ...
  private validationResult:
    | SafeParseSuccess<Specialities.Speciality>
    | SafeParseError<Specialities.Speciality>
    | undefined = undefined;
  private jsonOutput = "";
  ...
  private setupEventListeners() {
    setTimeout(() => {
      // Form submission
      const form = this.querySelector("#speciality-form");
      if (form) {
  ...
  private validateAndGenerateJSON() {
    ...
    const result = Specialities.Speciality.safeParse(this.formData);
  ...
  private collectFormData() {
    ...
    levelContainers.forEach((container) => {
      const level = container.querySelector(".level-select")
        ? ((container.querySelector(".level-select") as HTMLSelectElement)
            .value as Constants.SpecialityLevelName)
        : null;
      ...
      if (level) {
        ...
        buffContainers.forEach((buffContainer) => {
          const attribute = buffContainer.querySelector(".buff-attribute")
            ? (
  ...
  private render() {
    this.innerHTML = `
      <div class="spectrum spectrum-Typography">
        <h1 class="spectrum-Heading spectrum-Heading--sizeL">Speciality JSON Builder</h1>
        <p class="spectrum-Body spectrum-Body--sizeM">
          Use this form to create a valid Speciality JSON object. Fill in the fields and click "Generate JSON" to validate and create the JSON.
        </p>

        <form id="speciality-form" class="spectrum-Form">
          <div class="spectrum-Form-item">
            <label for="name" class="spectrum-FieldLabel">Speciality Name</label>
            <input type="text" id="name" class="spectrum-Textfield" required>
          </div>

          <h3 class="spectrum-Heading spectrum-Heading--sizeM">Speciality Levels</h3>
          <div id="levels-container" class="spectrum-Form-item">
            <button type="button" id="add-level" class="spectrum-Button spectrum-Button--primary">Add Speciality Level</button>
          </div>
