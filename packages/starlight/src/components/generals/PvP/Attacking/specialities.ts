...
export const PvPAttackingSpeciality = (
...
const specialityEval = (
    ...
    rValue += levels.reduce((sum: number, level) => {
      ...
      if (Array.isArray(level.buff)) {
        if (
          ...
          if (DEBUG) {
            console.log(
              `specialityEval ${role} ${speciality.name} for ${attribute} debuff: ${debuffAttribute}, level.level: ${level.level}, selections[levenNumber]: ${selections[levenNumber]}`
            );
          }
          sum += level.buff.reduce((sum2, buff) => {
            if (debuffAttribute) {
              if (debuffClass) {
                sum2 += PvPAttackingDebuff(buff, attribute, debuffClass);
              } else {
                throw new Error(
                  `when evaluating for a debuffAttribute, debuffClass must be set.`
                );
              }
            } else {
      ...
      if (DEBUG) {
        console.log(
          `specialityEval ${role} ${speciality.name} for ${attribute} debuff: ${debuffAttribute}, : level ${level.level} returning ${sum}`
        );
      }
      return sum;
    }, 0);
  ...
  if (DEBUG) {
    console.log(
      `specialityEval ${role} ${speciality.name} for ${attribute} debuff: ${debuffAttribute}, : returning ${rValue}`
    );
  }
  return rValue;
};
